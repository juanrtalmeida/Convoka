...
export class TeamService {
    static async generateTeams(convokaId, requesterId, numberOfTeams, roleRequirements) {
        ...
        for (const team of teams) {
            for (const member of team.members) {
                updatePromises.push(prisma.participant.update({
                    where: { id: member.id },
                    data: { teamId: team.id },
                }));
            }
    ...
    static async clearTeams(convokaId, requesterId) {
        const convoka = await prisma.convoka.findUnique({
            where: { id: convokaId },
        });
        if (!convoka || convoka.creatorId !== requesterId) {
