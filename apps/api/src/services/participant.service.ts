...
export class ParticipantService {
  static async joinConvoka(convokaId: string, userId: string, roles: string[] = []) {
    try {
      ...
      if (!convoka) throw new Error('Convoka não encontrada');
      ...
      if (existing) throw new Error('Usuário já está na Convoka');
  ...
  static async leaveConvoka(convokaId: string, userId: string) {
  ...
  static async manageParticipant(
    ...
    creatorId: string, 
    targetUserId: string, 
    action: 'UPDATE_PAYMENT' | 'MOVE_TO_WAITLIST' | 'MOVE_TO_CONFIRMED' | 'REMOVE',
    hasPaid?: boolean
  ) {
    ...
    switch (action) {
      case 'UPDATE_PAYMENT':
        if (typeof hasPaid !== 'boolean') throw new Error('Valor hasPaid inválido');
